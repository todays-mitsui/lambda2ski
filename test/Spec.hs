...
main = hspec $ do
  ...
  describe "Parser.def" $ do
    it "can parse Ident define" $ do
      let src = intercalate (singleton '\n') ["k = ^xy.y", "s = ^xyz.``xz`yz", "i = ``skk"]
      parse def "" src `shouldBe` Right (Ident "k", x :^ y :^ Var y)

  describe "Parser.context" $ do
    it "can parse Ident defines" $ do
      let src = intercalate (singleton '\n') ["k = ^xy.y", "s = ^xyz.``xz`yz", "i = ``skk"]
      parse P.context "" src
        `shouldBe` Right (Map.fromList [(Ident "i",Var (Ident "s") :$ Var (Ident "k") :$ Var (Ident "k")), (Ident "k", x :^ y :^ Var y), (Ident "s",x :^ y :^ z :^ Var x :$ Var z :$ (Var y :$ Var z))])
